...
void IRAM_ATTR SPIRenderer::draw()
{
  ...
  if (DrawNow_Flag == 0)
  {
    ...
    if (ilda->frames[frame_position].isBuffered == true && Pause_Flag == 0) // 如果只有缓存暂停，由于总共有8个buffer，投影还是会继续，会唤醒缓存
    {
      // 这一帧还没画完
  ...
  else
  {
    if (ilda->frames[0].number_records > 0) // 已经有数据点了
    {
      ...
      // TFTLCD反馈的x为0-480，y为0-320
      int y = 1024 + (instruction.x * 2048) / 320;
      int x = 3072 - (instruction.y * 2048) / 480;
