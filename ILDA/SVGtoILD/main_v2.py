...
def select_svg_file():
    ...
    if filename:
        process_svg_file(filename)

def process_svg_file(filename):
    ...
    ild_filename = filedialog.asksaveasfilename(defaultextension=".ild")
    write_ild(params, rframe, ild_filename, center)
...
def create_gui():
    ...
    window.title("SVG to ILDA Converter")  # 设置窗口标题

    global optimize_var, verbose_var, center_var
    optimize_var = tk.BooleanVar(value=True)
    verbose_var = tk.BooleanVar(value=True)
    center_var = tk.BooleanVar(value=True)

    optimize_check = tk.Checkbutton(window, text="Optimize", variable=optimize_var)
    verbose_check = tk.Checkbutton(window, text="Verbose", variable=verbose_var)
    center_check = tk.Checkbutton(window, text="Center", variable=center_var)

    button = tk.Button(window, text="Select SVG file", command=select_svg_file)
