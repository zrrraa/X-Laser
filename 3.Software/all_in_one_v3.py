...
CONFIG = {
    ...
    'flip_horizontal': False,  # 新增水平翻转
    'flip_vertical': False     # 新增垂直翻转
}
COLOR_MAP = {
    'red': 9,
    'yellow': 18,
...
# SVG 转 ILD 相关函数
# --------------------------------------
def svg_to_laser_json(svg_file, scale=1.0, tx=0, ty=0, flip_h=False, flip_v=False,
                      samples_per_unit=30, min_samples=5, endpoint_blanking=10):
    CONFIG.update({
        'manual_scale': scale,
        'manual_translate_x': tx,
        'manual_translate_y': ty,
        'flip_horizontal': flip_h,
        'flip_vertical': flip_v,
        'samples_per_unit': samples_per_unit,
        'min_samples': min_samples,
        'endpoint_blanking': endpoint_blanking
    })
    ...
    for segment in path:
        ...
        for t in np.linspace(0, 1, samples):
            ...
            if CONFIG['flip_horizontal']:
                x_base = canvas_size - x_base
            if CONFIG['flip_vertical']:
                y_base = canvas_size - y_base
...
def ilda_to_gif(ild_data, palettes, output_gif, canvas_size=800, fps=30):
    color_map = {
        ...
        36: (0, 255, 255),  # cyan
        45: (0, 0, 255),    # blue
        54: (255, 0, 255),  # magenta
        63: (255, 255, 255),# white
        64: (0, 0, 0)       # black
    }
    # 定义区间与对应颜色值
    intervals = [
        (-float('inf'), 9, color_map[9]),
        (9, 18, color_map[18]),
        (18, 27, color_map[27]),
        (27, 36, color_map[36]),
        (36, 45, color_map[45]),
...
class LaserShowConverter(tk.Tk):
    def __init__(self):
        ...
        self.samples_per_unit = tk.IntVar(value=30)
        self.min_samples = tk.IntVar(value=5)
        self.endpoint_blanking = tk.IntVar(value=10)
    ...
    def create_widgets(self):
        ...
        top_container.pack(fill=tk.BOTH, expand=True, padx=10, pady=0)
        ...
        # 新增翻转选项
        flip_frame = ttk.Frame(svg_group)
        flip_frame.grid(row=1, columnspan=3, pady=5)
        ttk.Checkbutton(flip_frame, text="水平翻转", variable=self.flip_h).pack(side=tk.LEFT, padx=5)
        ttk.Checkbutton(flip_frame, text="垂直翻转", variable=self.flip_v).pack(side=tk.LEFT, padx=5)
        
        # 新增参数配置
        param_frame = ttk.Frame(svg_group)
        param_frame.grid(row=2, columnspan=3, pady=5)
        ttk.Label(param_frame, text="采样密度:").pack(side=tk.LEFT)
        ttk.Entry(param_frame, textvariable=self.samples_per_unit, width=5).pack(side=tk.LEFT, padx=2)
        ttk.Label(param_frame, text="最小采样:").pack(side=tk.LEFT)
        ttk.Entry(param_frame, textvariable=self.min_samples, width=5).pack(side=tk.LEFT, padx=2)
        ttk.Label(param_frame, text="消隐点数:").pack(side=tk.LEFT)
        ttk.Entry(param_frame, textvariable=self.endpoint_blanking, width=5).pack(side=tk.LEFT, padx=2)
        
        # 原有平移和缩放控件
        ttk.Label(svg_group, text="X 平移:").grid(row=3, column=0, sticky="w", pady=2)
        ...
        ttk.Label(svg_group, text="Y 平移:").grid(row=4, column=0, sticky="w", pady=2)
        ...
        ttk.Label(svg_group, text="缩放倍数:").grid(row=5, column=0, sticky="w", pady=2)
        ...
        # 底部容器 - 串口传输区域
        bottom_container = ttk.LabelFrame(self, text="串口传输", padding="0")
        bottom_container.pack(fill=tk.X, expand=False, padx=10, pady=0)
        ...
        # 状态栏
        status_container = ttk.Frame(self, height=48)
        status_container.pack(side=tk.BOTTOM, fill=tk.X)
        status_container.pack_propagate(False)  # 禁止子组件调整容器大小

        # 状态栏标签（保持原有样式，添加垂直填充适配高度）
        self.status_lbl = ttk.Label(
            ...
            font=("Segoe UI", 9),
            anchor="nw",  # 左对齐
            padding=(2, 0)  # 水平方向添加少量内边距
        )
        self.status_lbl.pack(fill=tk.BOTH, expand=True)  # 填充容器空间
    ...
    def convert_svg_to_ild(self):
        ...
        try:
            ...
            tx = self.tx_var.get()
            ty = self.ty_var.get()
            samples = self.samples_per_unit.get()
            min_s = self.min_samples.get()
            blank = self.endpoint_blanking.get()
        except tk.TclError:
            messagebox.showerror("错误", "请输入有效的数值参数")
            return
        
        self.status_lbl.config(text=f"正在转换 SVG 到 ILD... 输出：{self.output_ild}")
        self.current_conversion_type = 'svg'
    ...
    def _convert_svg(self, svg_path, scale, tx, ty, flip_h, flip_v, samples, min_s, blank):
        """实际转换过程"""
        try:
            ilda_data = svg_to_laser_json(svg_path, scale, tx, ty, flip_h, flip_v, 
                                         samples, min_s, blank)
            if not ilda_data:
                raise ValueError("未生成有效数据")
            write_ilda_file(ilda_data, self.output_ild)
            success = ilda_to_gif(ilda_data, {}, self.output_gif, 200, 30)
