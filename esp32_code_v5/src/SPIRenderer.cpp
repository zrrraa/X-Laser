...
void setupRenderer()
{
  ...
  for (int i = 0; i < bufferFrames; i++)
  {
    ...
    memset(ilda->frames[i].records, 0, sizeof(ILDA_Record_t) * MAXRECORDS); // 清空初始化的内容
...
void IRAM_ATTR SPIRenderer::draw()
{
  ...
  if (DrawNow_Flag == 0)
  {
    ...
    if (ilda->frames[frame_position].isBuffered == true && Pause_Flag == 0) // 如果只有缓存暂停，由于总共有3个buffer，投影还是会继续，会唤醒缓存
    {
      // 这一帧还没画完
  ...
  {
    if (ilda->frames[0].number_records > 0) // 已经有数据点了
    {
      // 位置信号X&Y的处理和DAC输出
      const ILDA_Record_t &instruction = ilda->frames[0].records[draw_position];

      // TFTLCD反馈的x为0-200，y为0-200
      int y = 1024 + (instruction.x * 2048) / 200;
      int x = 3072 - (instruction.y * 2048) / 200;
      // ESP_LOGI(TAGRENDERER, "X: %d, Y: %d", x, y);
      ...
      // 由于画迹断开后的第一个点不亮，激光模组先于振镜处理，防止曲线封闭
      // ESP_LOGI(TAGRENDERER, "instruction.color: %d, instruction.status_code: %d", instruction.color, instruction.status_code);
      if ((instruction.status_code & 0b01000000) == 0)
      {
        if ((instruction.color & 0b11100000) == 0b11100000)
        {
          digitalWrite(PIN_NUM_LASER_R, LOW);
        }
        else
        ...
        if ((instruction.color & 0b00011000) == 0b00011000)
        {
          digitalWrite(PIN_NUM_LASER_G, LOW);
        }
        else
        ...
        if ((instruction.color & 0b00000111) == 0b00000111)
        {
          digitalWrite(PIN_NUM_LASER_B, LOW);
        }
        else
      ...
      // 画这一帧中的下一个点，画完则从头开始，循环这一帧
      draw_position++;
      // Serial.println(draw_position);
      // Serial.println(ilda->frames[0].number_records);
      if (draw_position > ilda->frames[0].number_records)
      {
        draw_position = 0;
      }
    }
...
void SPIRenderer::start()
{
  ...
  // xTaskNotifyGive(fileBufferHandle); // 会导致刚开始初始化时filebufferloop不能因为Pause_Flag进入阻塞，故注释掉
  delay(2000);
